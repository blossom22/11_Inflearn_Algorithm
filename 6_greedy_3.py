...
def solution(m, nums):
    ...
    start = end = i = 0     # 선의 start, end 값을 초기에는 0설정
    while i < n:
        while i < n and nums[i][0]<=start:  # start위치보다 nums[i][0]작거나 같으면 이런 선들중 길이가 가장 긴놈의 끝지점으로 end값 업데이트
            end = max(end, nums[i][1])
            i+=1 
        answer+=1
        if end==m:
            return answer
        start = end     # 만약 end가 m까지 안 왔으면, 다음 선을 찾아야하니까, start가 end로 바뀌어
    return answer
